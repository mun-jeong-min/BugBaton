...
Chroma connects through the Chrome DevTools Protocol, which grants broad control
over the attached browser profile. Treat the endpoint and every captured artifact
as sensitive.

## Reporting a Vulnerability

Use [GitHub private vulnerability reporting](https://github.com/mun-jeong-min/Chroma/security/advisories/new).
Do not open a public issue for a vulnerability, and do not attach a real Chroma
...
- Chroma is not a sandbox and does not authenticate a CDP endpoint.
...
- Reports remain local. Chroma does not upload artifacts, expose a tunnel, or send telemetry.
