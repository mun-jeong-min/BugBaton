...
export class CdpConnection {
  ...
  async open(timeoutMs = 5_000) {
    ...
    this.socket.addEventListener("error", () => this.#closePending(codedError("CDP_CONNECTION_FAILED", "CDP connection failed", { retryable: true, hint: "Run `bugbaton doctor` to inspect endpoint and session health." })));
    let timer;
    try {
      await Promise.race([
        new Promise((resolve, reject) => {
          this.socket.addEventListener("open", resolve, { once: true });
          this.socket.addEventListener("error", () => reject(codedError("CDP_CONNECTION_FAILED", `Could not connect to ${this.url}`, { retryable: true, hint: "Run `bugbaton doctor` to inspect endpoint and session health." })), { once: true });
        }),
        new Promise((_, reject) => {
          timer = setTimeout(() => reject(codedError("CDP_TIMEOUT", `Timed out opening CDP WebSocket after ${timeoutMs}ms`, { retryable: true, hint: "Run `bugbaton doctor` and retry if the endpoint is healthy." })), timeoutMs);
        }),
      ]);
    } finally {
      clearTimeout(timer);
  ...
  send(method, params = {}, timeoutMs = 10_000) {
    ...
    return new Promise((resolve, reject) => {
      const timer = setTimeout(() => {
        ...
        reject(codedError("CDP_TIMEOUT", `${method}: CDP response timed out after ${timeoutMs}ms`, { retryable: true, hint: "Run `bugbaton doctor`; retry if the session is healthy.", details: { method, timeoutMs } }));
      }, timeoutMs);
